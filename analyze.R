...
data.demograph <- filter(data.demograph, pie.id %in% pts.include.man) %>%
...
data.assessments <- filter(data.assessments, pie.id %in% pts.include.man)
...
data.lfts <- filter(data.labs.lfts.long, pie.id %in% pts.include.man)
...
data.apache <- filter(data.apache, pie.id %in% pts.include.man) %>%
    inner_join(tmp, by = "pie.id")
